...
const SEG =
  "px-1 py-[7px] text-center text-[11px] font-medium sm:px-3 sm:text-[13px] leading-[18px] tracking-[-0.01em] whitespace-nowrap";
...
export function SegmentedControl({
  ...
  return (
    <div
      role="radiogroup"
      aria-label={label}
      className={`relative inline-block select-none rounded-[9px] border border-stone-200 bg-stone-100/70 p-[3px] shadow-[inset_0_1px_2px_rgba(28,25,23,0.07)] dark:border-zinc-800 dark:bg-zinc-950/60 dark:shadow-[inset_0_1px_2px_rgba(0,0,0,0.45)] ${className}`}
    >
      <div
      ...
      >
        ...
        <motion.div
          aria-hidden
          className="pointer-events-none absolute inset-y-0 left-0 overflow-hidden rounded-[6px] bg-stone-800 shadow-[0_1px_2px_rgba(28,25,23,0.28)] dark:bg-zinc-100 dark:shadow-[0_1px_2px_rgba(0,0,0,0.5)]"
          ...
          >
            ...
            >
              {options.map((option) => (
                <span
                  key={option.value}
                  className={`${SEG} text-stone-50 dark:text-zinc-900`}
                >
                  {option.label}
                </span>
              ))}
